...
## State (P0 mid-build — SUPERSEDED by the code-complete section below)
...
- `outbox-worker.ts` — queue drain (claim via SECURITY DEFINER RPC, lease steal,
  bounded backoff, per-process latch). Wired into index.ts env-gated (doubly
  inert: needs the approval-gated P0 schema apply + `AMANDA_ENGINE_ENABLED=true`).
...
db backends + processTurnDb, EF v3 proposed, golden suite (33 scenarios, one
command: `npx vitest run apps/api/src/amanda-engine/golden/`), all tests green.
