/**
 * The internal scoring check (Stage 1, approved by Christian 2026-09-11). Runs the 11 fixture conversations through
 * the real scorer and judges them against his acceptance standard. It has NO database access: it reads no leads and
 * writes nothing. The model call is passed in: the admin route passes the server's own key path.
 */
...
import type { Band, Temperature } from './types';
 ...
 * missed. ACCEPTABLE: only same-band score differences, a band slip on a case marked minor, or quotes the verifier
 * caught and discarded (they can never raise a score or appear in the explanation, and are reported here).
 * GREEN: everything as expected and nothing discarded.
 */
...
export async function runScoringCheck(
  ...
  for (const fx of fixtures) {
    ...
    if (spent + worstCaseCostUsd(fx.input) > capUsd) {
      cases.push({ ...base, actual: null, ok: false, explanation: null, discarded: [], guards: [], inputTokens: 0, outputTokens: 0, costUsd: 0, error: 'not run: it could have passed the cost cap' });
