-- Amanda answer-first (2026-08-11) — amanda_search_properties v2: free-text PHRASE match.
--
-- "Tell me more about the one in Playa del Cura" refers to a listing by a phrase the
-- zone tables don't know. p_filters.q (extracted client-of-record-side by
-- parseSearchPhrase) now matches the phrase against listing TITLE (ILIKE) and city;
-- when q is present it replaces the town/zone gate (budget/beds/type gates still
-- apply). Everything else — status='active' only, safe columns only, HARD cap 3,
-- ref mode, is_test gate, no-wrong-town honesty — is unchanged from
-- 20260731095223. Same signature: CREATE OR REPLACE, no drop.
