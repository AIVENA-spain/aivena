...
// SHARED photo-selection: build the photos argument for renderEditable from an ordered list of images
...
export async function pickPhotos(m: z.infer<typeof EditableManifest>, imgs: (string | Buffer)[], templateId: string): Promise<string | Record<string, string>> {
  ...
  return toUri(imgs[templateId === "1" ? Math.min(1, imgs.length - 1) : 0]);
}
