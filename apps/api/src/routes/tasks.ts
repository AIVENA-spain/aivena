...
route.get('/', async (c) => {
  ...
  const rawType = c.req.query('type') ?? 'suggested_reply';
  const allTypes = rawType === 'all';
  const taskTypesCsv = allTypes
    ? ''
    : rawType.split(',').map((t) => t.trim()).filter((t) => /^[a-z_]{1,40}$/.test(t)).join(',') || 'suggested_reply';
  ...
  const result = await tx.execute(sql`
    ...
    WHERE (${allTypes}::boolean OR dt.task_type = ANY (string_to_array(${taskTypesCsv}, ',')))
