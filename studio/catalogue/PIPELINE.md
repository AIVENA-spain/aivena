...
- **#7 proof** (Discover Your Dream — MULTI-PHOTO; `7.editable.json`, `assets/7/`): 5/5 — 8 `<text data-editable>`; **all 4 photo slots fill (hero + 3 thumbnails)**; roles recolour independently. `renderEditable` now supports `photo_slots: [{token}]` (hero + thumbnails) alongside single `photo_token` (back-compatible; #1/#11 unchanged). Remaining #7 need: the right-panel feature LIST (4 Bedrooms/…) is left baked for first proof (only the header is re-rendered).
