...
interface AdjLayer { id: string; label: string; faithful_label?: string; improvement_reason?: string | null; score: number; selected_font: string | null; candidates: { font: string; score: number }[]; }
...
export function extractManifest(templateId: string): { bindings: ManifestBindings; outDir: string } {
  ...
  for (const slot of manifest.text_slots) {
    ...
    const improvement = !!adj?.improvement_reason;
    const faithful_label = adj?.faithful_label ?? adj?.label ?? "(no layer)";
    ...
    else if (improvement && agrees) note = `production improvement: manifest font ${slot.font} (${manifest_family}) == adjudicator-selected ${adj.selected_font}; faithful=${faithful_label}. ${adj.improvement_reason}`;
    ...
    else note = `NOT resolved: adjudicator label '${adj.label}' (score ${adj.score}); manifest sets ${slot.font} (${manifest_family}) — closest active candidate ${adj.candidates?.[0]?.font ?? "?"} ${adj.candidates?.[0]?.score ?? ""}; needs seed/source truth (Q9)`;
    ...
    bindings.push({
      ...
      confirmed, improvement, faithful_label, agrees_with_manifest: agrees, note,
    });
  }

  const unresolved = bindings.filter((b) => !b.confirmed && !b.improvement).map((b) => b.slot_id);
  const result = ManifestBindings.parse({
    ...
    all_confirmed: bindings.every((b) => b.confirmed),
...
function bindingsMd(b: ManifestBindings): string {
  let s = `# Manifest font bindings — ${b.template_id}\n\nadjudication status: \`${b.adjudication_status}\` · vault \`${b.vault_version}\` · all confirmed: ${b.all_confirmed} · all resolved: ${b.all_resolved} · unresolved: ${b.unresolved_slots.join(", ") || "(none)"}\n\n`;
  s += `| slot | manifest font (family) | adjudicator (faithful) | confirmed | improvement | vault | note |\n|---|---|---|---|---|---|---|\n`;
  for (const x of b.bindings) {
    s += `| ${x.slot_id} | ${x.manifest_font} (${x.manifest_family}) | ${x.adjudicator_label} ${x.adjudicator_score} (${x.faithful_label}) | ${x.confirmed ? "yes" : "NO"} | ${x.improvement ? "yes" : "—"} | ${x.vault_id ?? "—"}/${x.vault_status ?? "—"} | ${x.note.replace(/\|/g, "/")} |\n`;
  }
  return s + "\n";
}
...
if (require.main === module) {
  ...
  try {
    ...
    for (const b of bindings.bindings) console.log(`  ${b.slot_id.padEnd(10)} ${b.manifest_font.padEnd(18)} adj=${b.adjudicator_label.padEnd(20)} confirmed=${b.confirmed ? "yes" : "NO "} improvement=${b.improvement ? "yes" : "no "} vault=${b.vault_id ?? "—"}`);
    console.log(`\n  all_fonts_vault_backed=${bindings.all_fonts_vault_backed} all_confirmed=${bindings.all_confirmed} all_resolved=${bindings.all_resolved} unresolved=[${bindings.unresolved_slots.join(",")}]`);
