...
## BATCH 2 — next templates (2026-07-02, control tower assigned "next 4")
Catalogue has no explicit priority order, so selected for best variety: **#5, #10, #14, #15**. Built via the SAME general engine (`renderEditable` + `deriveSlots` + `visualQA`) + rendered on the 3 real property types (chalet/apartment/bungalow).
- **✅ #5 Listing (DARK)** — BUILT TO STANDARD, ready for review. Photo bg + dark overlay + white text; 2-line serif title `{Type} in {City}`, price (`PRICE: €…`), 3-stat row (house/bed/bath icons KEPT — universally correct), website pill. 9 editable `<text>`; QA PASS on all 3 types; recolour proof PASS. `manifest/templates/5.editable.json`, `assets/5/`.
- **✅ #14 Just Sold (LIGHT)** — BUILT TO STANDARD, ready for review. `JUST SOLD!` headline + property photo (mid) + 3-stat row (icons kept) + contact block (sold-location + agency phone/web) + agency-name block that REPLACES the baked "Snag Space" placeholder logo. 9 editable `<text>`; QA PASS on all 3 types; recolour proof PASS. `manifest/templates/14.editable.json`, `assets/14/`.
- **⚠️ #10 New Listing (multi-photo)** — NOT built. Structural blocker: the dominant `NEW`/`LISTING` type + price pill are **baked teal outlined paths** → they don't follow the agency colour-wheel (navy/gold) without re-rendering a heavy condensed font (needs_seed). Editable property data (4 photos hero+3 thumbs, price, body) is feasible, but the template's identity colour is baked → can't match the approved recolour standard cleanly.
- **⚠️ #15 Luxury Living (2-photo)** — NOT built. Structural blocker: the 7-field spec list needs data we don't have (Lot Area / Garage / Dining / Bonus) — we have only Floor-Area/Beds/Baths → 4/7 rows empty; + a **script "Made Affordable" subtitle** with no matching vault font (Poppins/LibreCaslon/Prata/Tinos).
- **Recommendation (control-tower decision):** keep the batch at 4 solid by **swapping #10/#15 → cleaner-fitting catalogue templates** (e.g. **#2 Luxury villa** + **#6 Open House gallery**, both single-accent single-/multi-photo designs), OR proceed with #10/#15 under documented caveats. Cross-type proof sheet: `out/realprop/review/batch2_new_templates.png`.
