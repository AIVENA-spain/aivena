...
- **✅ #3 Luxury Villa (PHOTO-DOMINANT)** — property photo fills the canvas + legibility scrim; white `LUXURY {Type}` title, generated-from-facts description, baked dark bottom bar with 3-stat row (icons kept) + website. 8 editable `<text>`. Caveat: the source's script accent on the type word isn't reproduced (no script font — Libre Caslon Display used).
- **✅ #6 Open House Gallery (4-photo, LIGHT)** — 4-photo grid + agency handle + a 2-line booking CTA (`BOOK A VIEWING` + phone — **no invented open-house date**; the agency schedules it). The "Open House" script title stays baked (fixed identity; no script font). 3 editable `<text>` + 4 photos.
